...
ESX.ShowNotification = function(msg)
	...
	--TriggerEvent('mNotify:sendtheNotification', 'Ειδοποίηση', msg, "left", "mid", false)
	TriggerEvent("esx_pushNotification:sendNotification", msg)
	--TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = msg, length = 3500})
	-- TriggerEvent('MJ_Notify', "", msg, 3000, 'info')
end
...
ESX.ServerCallbacksValid = {}

ESX.TriggerServerCallback = function(name, cb, ...)
	ESX.ServerCallbacks[ESX.CurrentRequestId] = cb
	ESX.ServerCallbacksValid[ESX.CurrentRequestId] = true
...
ESX.Game.SpawnVehicle = function(modelName, coords, heading, cb)
	...
	Citizen.CreateThread(function()
		...
		--[[ while not HasCollisionLoadedAroundEntity(vehicle) and timeout < 2500 do
			RequestCollisionAtCoord(coords.x, coords.y, coords.z)
			Citizen.Wait(0)
			timeout = timeout + 1
		end ]]
...
RegisterNetEvent('esx:serverCallbackCheck')
AddEventHandler('esx:serverCallbackCheck', function(requestId, ...)
	if not ESX.ServerCallbacksValid[requestId] then
		TriggerServerEvent('esx:triggeredCallbackMpura123',requestId,...)
	else
		ESX.ServerCallbacksValid[requestId] = nil
	end
end)
...
ESX.GetPersonalAchCategory = function(name)
	if Config.Achievements['personal'][name] then
		return Config.Achievements['personal'][name]
	else
		return "Not Found"
	end
end

ESX.GetPersonalAchCategories = function()

	local Categories = {}
	for name,v in pairs(Config.Achievements['personal']) do
...
ESX.DoesPersonalAchCategoryExist = function(name)
...
ESX.GetPersonalAchsUsingCategory = function(catname,minify)
	if not ESX.DoesPersonalAchCategoryExist(catname) then
		return {}
	else
		local achs = {}
		for name, ach in pairs(Config.Achievements['personal']) do
			if ach.category == catname then
				if minify == nil then
					table.insert(achs,ach)
				else
...
ESX.DoIHaveItem = function(name,count)
	if ESX.PlayerData.inventory then 
		-- if ESX.PlayerData.inventory[name] and ESX.PlayerData.inventory[name].count and ESX.PlayerData.inventory[name].count >= count then 
		-- 	return true 
		-- else
		-- 	return false 
		-- end
		for k,v in pairs(ESX.PlayerData.inventory) do
			if v.name == name then
				if v.count and v.count >= count then
					return true
				else
					return false
				end
...
ESX.GetItemLabel = function(item)
	if ESX.PlayerData.ItemLabels and ESX.PlayerData.ItemLabels[item] then 
		return ESX.PlayerData.ItemLabels[item]
	else
		return item 
	end
end